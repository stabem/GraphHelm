...
//! while the reader of a failure gets `reached the store as a real budget; an              unreadable
//! deadline must be refused`.
//!
//! **Why this file is not called `source_invariants.rs`.** This crate already has one, guarding a
//! different property entirely -- that the completion builder's key vocabulary is known to its
//! collector. Adding these tests there would make a file with two owners, and in a two-owner file
//! a merge can drop one side's work without ever raising a conflict. A separate file costs a
//! filename and removes that whole failure mode.
//!
...
/// prose that a reader meets on a failure, including the reason strings in `CONTENT_FREE_KEYS`,
/// which are interpolated into an assertion message rather than being data the tests compare.
/// None of the DATA species that force a role exemption elsewhere occurs in this crate. That is a
/// measurement, and if one ever lands here the exemption belongs beside it, by ROLE and never
/// per-file.
...
/// root would leave the scan above silently green.
#[test]
