...
//! while the reader of a failure gets `a stage nobody bounded has NO deadline, and          reading
//! absence as zero`.
//!
...
///
/// This crate reaches the state where the root assertions matter before any of the others, and
/// the reason is about GROWTH rather than today's counts.
///
/// The current population is eleven files under `src/` and four under `tests/`, including this
/// guard: fifteen total. Losing `tests/` therefore drops the count from fifteen to eleven today.
/// Four ordinary additions under `src/` would make a broken src-only walk reach fifteen again,
/// which is why the root assertions remain load-bearing instead of treating the exact floor as a
/// permanent proof that both roots were visited.
#[test]
