...
RUN=C:/Users/example/AppData/Local/Temp/m08-judge6b
ART=$WT/docs/acceptance/m08-rejudge6-2026-08-18
PORT=42223
...
def post(path, body, key):
    ...
    return json.load(urllib.request.urlopen(request, timeout=1200))
...
def alarm():
    ...
    try:
        armed = post("/wake-lease", {"sessionId": "operator-m07",
                                     "rendezvousId": "m07-operator"}, "m08-arm6")
...
try:
    reply = post("/start", body, "m08-judge6b-start")
except urllib.error.HTTPError as error:
