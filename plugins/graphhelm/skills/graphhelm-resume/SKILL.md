...
2. Identify the nearest unresolved user promise. Prefer an action that removes its blocker or obtains the missing observer. Choose a second viable action with a real tradeoff, such as preparing a specific proof checklist or inspecting a named work record while a blocker remains. Ground both options in the supplied goal and evidence. If a source is inaccessible, the second action must still be possible with available information or by asking the owner for the missing record; do not propose inspecting that inaccessible source as if it were available. Never write a generic "work on another task" option unless that task is named. Do not invent a task, cost saving, pass, merge, or deployment.
