...
# token-bench oracle for task 1145: the fix's test file (c4cbae8e) restricted to what issue #1145 ASKED.
# Removed: the guard-text assertion naming the real fix's variable, the inline-join half of the
# start counts, and the coverage cell. Behavioural cells are untouched.
...
try {
    ...
    Assert-True ($startSlice.Contains("Start-PostgresStageEarly -Name 'PostgreSQL ignored matrix'") -and $startSlice.Contains("Start-PostgresStageEarly -Name 'PostgreSQL matrix under a non-C collation' -Locale")) `
...
\s*\(Test-StageOverlapped -Records \`$stageRecords -Name 'PostgreSQL matrix under a non-C collation'\)") `
    ...
    foreach ($case in $zeroLikeCases) {
        ...
        Invoke-Expression $startSlice
        Invoke-Expression $joinSlice
        $zeroLikeCounts[$case.Name] = @((Get-Content -LiteralPath $startedMarker -ErrorAction SilentlyContinue) | Where-Object { $_ -eq 'early' }).Count
        Remove-Item -LiteralPath $startedMarker -Force -ErrorAction SilentlyContinue
    }
    } finally {
    ...
    Assert-True (@($startedOnFailure | Where-Object { $_ -eq 'early' }).Count -eq 0) 'a failed artifact build starts no PostgreSQL cluster'
    Assert-True (@($startedOnUnknown | Where-Object { $_ -eq 'early' }).Count -eq 0) 'an artifact manifest with no build result starts no PostgreSQL cluster'
